...
init_mdl_tbl <- function(mdl_object, tf="", var="") {
  ...
  cap_text <- paste("Response:", var, end_text[tf]) # for the table caption
...
mdl_result <- function(model, conf_level = 0.95, type="param", caption = "")
{ # param: parameter level metrics
  # overall: for overall model metrics
  
  if(type == "param") {
    caption <- ifelse(caption=="", "Parameter estimates", caption)
    ...
    caption <- ifelse(caption=="", "Overall Model Performance Results", caption)
    # extracting results at the model level
    out  <- model %>%
      glance() %>%
      select(r.squared,adj.r.squared, sigma, F.statistic=statistic, p.value) %>%
      mutate(MSE=sigma^2, .keep="unused", .after="adj.r.squared") %>%
      kable(format="latex", booktabs = T, linesep ="", caption=caption) %>%
      kable_styling(font_size = 10,  latex_options = c("HOLD_position"))
  }
  return(out)
}
